...
static void draw_floor_pixels(floor_ctx_t *ctx,
...
static void render_floor_line(floor_ctx_t *ctx, int y, player_t *player)
{
    float fov;
    float pos_z;
    int p;
    ...
    step.floor_y = (-1 * *player->y) + row_dist * ctx->dirs->dir_y0;
    step.step_x = row_dist * (ctx->dirs->dir_x1 - ctx->dirs->dir_x0) /
        (float)ctx->width;
    step.step_y = row_dist * (ctx->dirs->dir_y1 - ctx->dirs->dir_y0) /
        (float)ctx->width;
    draw_floor_pixels(ctx, &step, y);
}

void render_floor(player_t *player, sfVector2i screen, wolf_context_t *ctx)
{
    sfImage *img;
    tex_info_t tex;
    floor_dirs_t dirs;
    ...
    img = ctx->cmap->floor->image;
    if (!img)
        return;
    tex.image = img;
    tex.width = sfImage_getSize(img).x;
    tex.height = sfImage_getSize(img).y;
    dirs = compute_dirs(*player->angle, player->param.fov);
    floor.player = player;
    floor.tex = &tex;
    floor.dirs = &dirs;
    floor.width = screen.x;
    floor.height = screen.y;
    for (y = screen.y / 2 + 1; y < screen.y; y++)
        render_floor_line(&floor, y, player);
}
