...
static player_t *get_nearest_visible_enemy(bst_context_t *context, player_t *p)
{
    float nd = 0.;
    player_t *nearest = NULL;
    int xout = 0;
    double dout = 0.f;

    for (int i = 0; i < context->n_players; i++) {
        if (context->players[i]->equipe == p->equipe
            || !project_stuff_on_screen(p, (sfVector2f){*context->players[i]->x,
            *context->players[i]->y}, &xout, &dout)) {
            continue;       
        }
        if (dout < p->zbuffer[(int)(p->param.size.x * (p->param.size.y / 2)) + xout]) {
            if (dout < nd || nearest == NULL) {
                nd = dout;
                nearest = context->players[i];
            }
        }
    }
    return nearest;
...
{
    player_t *nenemy = get_nearest_visible_enemy(context, player);
