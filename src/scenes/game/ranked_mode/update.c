...
static void check_endgame(bst_context_t *context)
{
    ...
    if (get_npfteam(context, 0) <= 0 && !context->bs->is_fired) {
        made_team_win(context, DEF_TEAM);
    } else if (get_npfteam(context, 1) <= 0) {
        made_team_win(context, ATK_TEAM);
    }
}
...
void update_player_ranked(bst_context_t *context, player_t *player)
{
...
void ranked_update(bst_context_t *context)
{
    ...
    for (int i = 0; i < context->n_players; i++) {
        update_player_ranked(context, context->players[i]);
    }
