...
int ranked_init_map(bst_context_t *context)
{
    if ((context->wins.x + context->wins.y > 0)) {
        for (int i = 0; i < context->n_players; i++) {
            context->players[i]->equipe = !context->players[i]->equipe;
            spawn(context->players[i], context);
        }
        if (!((context->wins.x + context->wins.y + 1) % 5)) {
            for (int i = 0; i < context->n_map; i++) {
                if (context->cmap == context->maps[i]) {
                    context->cmap = context->maps[(i + 1) % context->n_map];
                    break;
                }
            }
        }
